...
public class BacteriaControllerOrange : MonoBehaviour
{
    [Header("Movement")]
    ...
    private void Awake()
    {
        ...
        Debug.Log($"[DEBUG][Orange] Awake → VFX ready: {hitPS != null}");
    }
    ...
    public void Hit()
    {
        if (isHit)
        {
            Debug.Log("[DEBUG][Orange] Hit() diabaikan (isHit == true)");
            return;
        }
        ...
        Debug.Log("[DEBUG][Orange] Bakteri akan dihancurkan 3 detik lagi");
    }
    ...
    private void TrySpawnAtom()
    {
        if (atomPrefab == null)
        {
            Debug.LogWarning("[DEBUG][Orange] atomPrefab belum di-assign");
            return;
        }

        if (Random.value > atomDropChance)
        {
            Debug.Log("[DEBUG][Orange] Atom tidak drop (chance gagal)");
            return;
        }
        ...
        Debug.Log("[DEBUG][Orange] Atom drop berhasil (mode bakteri)");
    }
    ...
    public void ProjectileHit(int damage)
    {
        ...
        Debug.Log($"[DEBUG][Orange] Damage={damage}, HP sisa={health}");
    ...
    public void ProjectileDead()
    {
        ...
        Debug.Log("[DEBUG][Orange] ProjectileDead()");
        Hit();
    }
}
