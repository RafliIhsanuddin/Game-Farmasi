...
public class ProtozoaController : MonoBehaviour
{
    ...
    private void TrySpawnAtom()
    {
        if (atomPrefab == null)
        {
            Debug.LogWarning("[DEBUG][Protozoa] atomPrefab belum di-assign");
            return;
        }

        if (Random.value > atomDropChance)
        {
            Debug.Log("[DEBUG][Protozoa] Atom tidak drop (chance gagal)");
            return;
        }
        ...
        Debug.Log("[DEBUG][Protozoa] Atom drop berhasil");
    }
